...
_FLAT_CHUNK = 32768
# Large-input chunk. On this build, an unmasked tile of 131072 lanes with
# buffer_size_limit=2048 is empirically exact (all-ones structural test at
# M=16.7M returns the integer numel with zero lane drop, and randn matches an
# fp64 reference within 3e-3). Using it for large M cuts the partial count 4x
# and the launched-block setup overhead, lifting flat-reduce throughput ~15-22%
# (validated 2026-09-18, probe_chunk_bw.py). Kept separate from _FLAT_CHUNK so
# the conservative 32768 point still serves the 32K..128K mid tier.
_FLAT_CHUNK_BIG = 131072
...
def _launch_sum_flat(inp, out, acc_dtype):
    ...
    if M <= 8192:
        ...
        return
    # CHUNK selection by size tier. CHUNK = 8192 keeps mid-M inputs
    # (8K < M < 32K) on the cheap exact tiles; CHUNK = 32768 (with
    # buffer_size_limit=2048) halves the partial count for large inputs;
    # CHUNK = 131072 (empirically exact, see _FLAT_CHUNK_BIG) is used once
    # M >= 131072 to cut partials 4x further and raise throughput. The tail is
    # either a single-shot masked tile (<= 8192 lanes, no staging) or a
    # zero-padded staged tile.
    if M >= _FLAT_CHUNK_BIG:
        CH = _FLAT_CHUNK_BIG
    elif M >= _FLAT_CHUNK:
        CH = _FLAT_CHUNK
    else:
